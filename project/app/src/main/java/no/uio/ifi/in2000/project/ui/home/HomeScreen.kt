...
fun UVScale(uvIndex: Float, modifier: Modifier = Modifier) {
    val gradientColors = listOf(
        Color(0xFF14FC00), // Low
        Color(0xFFDEEF17), // Moderate
        Color(0xFFFFAA06), // High
        Color(0xFFFD6C06), // Very High
        Color(0xFFFB0606), // Extreme
        Color(0xFF9E06FB)  // Above Extreme
    )
    ...
    Box(
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            val positionX = (uvIndex.coerceIn(0f, 11f) / 11f) * size.width
            drawCircle(
                color = Color.White,
                center = Offset(positionX, size.height / 2),
                radius = 8.dp.toPx()
            )
        }
    }
