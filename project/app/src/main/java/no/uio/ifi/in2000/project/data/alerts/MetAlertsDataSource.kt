...
data class MetAlertsDataSource(private val path: String = "https://gw-uio.intark.uh-it.no/in2000/") {
    ...
    suspend fun getAlerts(county: String, lang: String): MetAlertsResponse? { //finnes mange andre parametere, la til lang!
        return try {
            //val httpResponse = client.get("weatherapi/metalerts/2.0/current.json") //Guaranteed to have data
            val httpResponse = client.get("weatherapi/metalerts/2.0/current.json?county=$county&lang=$lang") //Most likely no warnings. (Minimal data)
